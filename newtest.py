...
class DashboardApp:
    def __init__(self):
        # Initialize the Flask server with session support
        self.server = Flask(__name__)
        self.server.secret_key = os.environ.get('SECRET_KEY', 'default-dev-key-change-in-production')
    ...
    def _setup_layout(self):
        self.app.layout = dbc.Container([
            ...
            html.H2("Command Analysis Dashboard", className="mt-3 mb-4"),
            ...
            # Dashboard content - hidden until a file is uploaded
            html.Div(id='dashboard-content', style={'display': 'none'}, children=[
                # Reset Button
                dbc.Row([
                    dbc.Col([
                        dbc.Button("Reset View", id="reset-button", color="secondary", className="mb-3")
                    ], width={"size": 2, "offset": 10}, className="text-right")
                ]),

                # 1. Address Map and Stats Display (side by side) - Modified layout
                dbc.Row([
                    dbc.Col([
                        dcc.Graph(id="address-map")
                    ], width=10),  # Increased from md=8 to width=10 (80% of screen)
                    dbc.Col([
                        html.Div([
                            html.H4("Command Statistics", className="mb-3"),
                            html.Div(id="stats-display", className="stats-panel")
                        ], className="h-100 d-flex flex-column justify-content-center")
                    ], width=2)  # Decreased from md=4 to width=2 (20% of screen)
                ]),
                ...
                # 3. Command Count Bar Chart (replacing distribution plot)
                dbc.Row([
                    dbc.Col([
                        dcc.Graph(id="cmd-count-chart")
                    ], width=12)
                ]),
                ...
                dbc.Row([
                    dbc.Col([
                        dcc.Graph(id="chunk-size-charts")
                    ], width=12)
                ]),

                dcc.Store(id="time-range"),
                dcc.Store(id="index-range"),
                dcc.Store(id="last-trigger"),
                dcc.Interval(id="init-trigger", interval=100, n_intervals=0, max_intervals=1),
            ]),
        ], fluid=True)
    ...
    def _setup_callbacks(self):
        ...
        @self.app.callback(
            Output('user-id', 'data'),
            Input('user-id', 'modified_timestamp'),
            State('user-id', 'data')
        )
        ...
        @self.app.callback(
            ...
            Output('init-trigger', 'n_intervals'),
            Input('upload-data', 'contents'),
            State('upload-data', 'filename'),
            State('user-id', 'data')
        )
        def update_output(contents, filename, user_id):
            if contents is None:
        ...
        @self.app.callback(
            ...
            Output("index-range", "data"),
            Output("last-trigger", "data"),
            Input("init-trigger", "n_intervals"),
            Input("main-plot", "relayoutData"),
            Input("address-map", "relayoutData"),
            Input("reset-button", "n_clicks"),
            State("time-range", "data"),
            State("index-range", "data"),
            State("last-trigger", "data"),
            State('user-id', 'data'),
        ...
        def update_range_stores(n_intervals, relayout_main, relayout_addr, reset_clicks,
            ...
            if trigger == "init-trigger" or trigger == "reset-button":
            ...
            if trigger == "main-plot":
                ...
                if time_range:
                    ...
                    return time_range, idx_range, "main-plot"

            # Handle address map index-based updates
            if trigger == "address-map":
                ...
                if idx_range:
                    ...
                    return time_range, idx_range, "address-map"
        ...
        @self.app.callback(
            Output("cmd-count-chart", "figure"),
            Output("chunk-size-charts", "figure"),
            Input("time-range", "data"),
            State('user-id', 'data'),
            prevent_initial_call=True
        )
