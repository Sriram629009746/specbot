...
class DashboardApp:
    ...
    def clear_upload_dir(self, upload_id):
        """Delete all files in a user's upload directory"""
        dir_path = os.path.join(UPLOAD_FOLDER_ROOT, upload_id)
    ...
    def get_latest_upload_dir(self):
        """Returns the most recently modified upload directory inside UPLOAD_FOLDER_ROOT"""
        if not os.path.exists(UPLOAD_FOLDER_ROOT):
            return None

        subdirs = [
            os.path.join(UPLOAD_FOLDER_ROOT, name)
            for name in os.listdir(UPLOAD_FOLDER_ROOT)
            if os.path.isdir(os.path.join(UPLOAD_FOLDER_ROOT, name))
        ]
        if not subdirs:
            return None
    ...
    def get_upload_layout(self):
        ...
        return html.Div([
                ...
                dbc.Col([
                    dcc.Upload(
                        ...
                        style={
                            ...
                            'textAlign': 'center',
                            'margin': '10px',
                            'display': 'none'  # Hidden in favor of dash-uploader
                        },
                        multiple=False,
                        className="upload-area"
                    ),
                    # Dash Uploader for large files
                    html.Div(
                        du.Upload(
                            id='latency-dash-uploader',
                            max_file_size=10240,  # 10GB max file size
                            filetypes=['log', 'txt', 'csv'],
                            upload_id=None,  # Will be set dynamically
                            #upload_id="default-user",  # ✅ FIXED: use a string, not a lambda
                            default_style={
                                'width': '100%',
                                'height': '60px',
                                'lineHeight': '60px',
                            ...
                            text="Drag and Drop or Select a Log File",
                            pause_button=True,
                            cancel_button=True,
                        ),
                        id='dash-uploader-container'
                    ),
            ...
            dcc.Store(id='latency-uploaded-filepath'),
            dcc.Store(id='latency-upload-id'),
            ...
            html.Div(id="latency-app-container"), # Placeholder for the real app layout
    ...
    def register_callbacks(self, app):
        ...
        @app.callback(
            [Output('latency-dash-uploader', 'upload_id'),
             Output('latency-user-id', 'data')],
            Input('latency-init-trigger', 'n_intervals')
        )
        def initialize_uploader_and_user(n_intervals):
            #print(n_intervals)
            print(f"in init with {ctx.triggered_id} and n_intervals {n_intervals}")
            if n_intervals == 0:
                # Generate a new user ID
                user_id = str(uuid.uuid4())
                # Set up user-specific upload directory
                upload_dir = os.path.join(UPLOAD_FOLDER_ROOT, user_id)
                if not os.path.exists(upload_dir):
                    print(f"\n\ncreating new user dir: {user_id}")
        ...
        '''
        # Handle uploaded file with dash-uploader
        @app.callback(
            [Output('latency-upload-status', 'children', allow_duplicate=True),
             Output('latency-uploaded-filepath', 'data'),
             Output('latency-process-button', 'disabled', allow_duplicate=True),
             Output('upload-loading-output', 'children'),
             # Clear plots on upload start
            ...
            [Input('latency-dash-uploader', 'isCompleted'),
             Input('latency-dash-uploader', 'fileNames'),
             Input('latency-dash-uploader', 'upload_id')],
            [State('latency-user-id', 'data'),
             State('latency-uploaded-filepath', 'data')],
            prevent_initial_call=True
        )
        def handle_upload_complete(is_completed, filenames, upload_id, user_id, previous_filepath):
            # Create empty figure for clearing
            empty_fig = go.Figure()

            # Determine which input triggered the callback
            ctx_msg = callback_context.triggered[0]['prop_id'].split('.')[1]

            # Handle fileNames change - this occurs at the start of upload
            if ctx_msg == 'fileNames' and filenames:
                if len(filenames) > 0:
                    upload_message = dbc.Alert(
                        f"Uploading {filenames[0]} in progress...",
                        color="info"
                    )

                    # Clear any previous plots/files
                    if previous_filepath and os.path.exists(previous_filepath):
                        try:
                            # Don't delete the file during upload, just clear the plots
                            pass
                        except Exception as e:
                            print(f"Error clearing previous file: {e}")

                    # Clear all plots and hide dashboard content
                    return upload_message, no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {
                        'display': 'none'}

            # Handle completed upload
            if ctx_msg == 'isCompleted' and is_completed and filenames:
                if len(filenames) > 0:
                    filename = filenames[0]
                    upload_dir = os.path.join(UPLOAD_FOLDER_ROOT, user_id)
                    filepath = os.path.join(upload_dir, filename)

                    # Clean up previous file if it exists and is different from the current one
                    if previous_filepath and previous_filepath != filepath and os.path.exists(previous_filepath):
                        try:
                            os.remove(previous_filepath)
                            print(f"Deleted previous file: {previous_filepath}")
                        except Exception as e:
                            print(f"Error deleting previous file: {e}")

                    upload_message = dbc.Alert(
                        [
                            html.H5(f"Upload complete: {filename}", style={'color': 'green'}),
                            html.P("Click 'Process Log File' to analyze the data")
                        ],
                        color="success",
                        dismissable=True
                    )

                    return upload_message, filepath, False, None, empty_fig, empty_fig, empty_fig, empty_fig, {
                        'display': 'none'}
        ...
        @app.callback(
            Output("latency-upload-status", "children", allow_duplicate=True),
            Output("latency-uploaded-filepath", "data", allow_duplicate=True),
            Output("latency-process-button", "disabled", allow_duplicate=True),
            ...
            Output("latency-dashboard-content", "style", allow_duplicate=True),
            Input("latency-dash-uploader", "isCompleted"),
            State("latency-dash-uploader", "fileNames"),
            prevent_initial_call=True
        )
        def handle_upload_complete(is_completed, filenames):
            empty_fig = go.Figure()

            if not is_completed or not filenames:
                return "Upload not complete or no file selected.", no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {
                    "display": "none"}

            latest_dir = self.get_latest_upload_dir()
            if not latest_dir:
                return "No upload directory found.", no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {
                    "display": "none"}

            filepath = os.path.join(latest_dir, filenames[0])
            if not os.path.exists(filepath):
                return f"File not found: {filepath}", no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {
                    "display": "none"}

            # Optional: clear previous plots/state here if needed

            print(filenames[0])
            for filename in os.listdir(latest_dir):
                file_path = os.path.join(latest_dir, filename)
                if os.path.isfile(file_path) and filename not in [filenames[0]]:
                    os.remove(file_path)

            upload_message = dbc.Alert(
                [
                    html.H5(f"Upload complete: {filenames[0]}", style={'color': 'green'}),
                    html.P("Click 'Process Log File' to analyze the data")
                ],
                color="success",
                dismissable=True
        ...
        def handle_upload_complete(is_completed, filenames, upload_id, user_id):
            print(f"in handle with {ctx.triggered_id}")
            empty_fig = go.Figure()
            triggered = callback_context.triggered[0]['prop_id'].split('.')[1]

            print(triggered)
            print(upload_id)
            # Start of upload — show status + clear plots + delete old file(s)
            if triggered == "fileNames" and filenames:
                print(f"upload id: {upload_id}")
                print(upload_id)
                self.clear_upload_dir(upload_id)

                msg = dbc.Alert(f"Uploading {filenames[0]} in progress...", color="info")
                return msg, no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {'display': 'none'}

            # End of upload — file should now exist
            if triggered == "isCompleted" and is_completed and filenames:
                latest_dir = self.get_latest_upload_dir()
                if not latest_dir:
                    msg = dbc.Alert("No upload directory found.", color="danger")
                    return msg, no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {'display': 'none'}

                filepath = os.path.join(latest_dir, filenames[0])
                if not os.path.exists(filepath):
                    msg = dbc.Alert(f"File not found: {filepath}", color="danger")
                    return msg, no_update, True, None, empty_fig, empty_fig, empty_fig, empty_fig, {'display': 'none'}
                ...
                return msg, filepath, False, None, empty_fig, empty_fig, empty_fig, empty_fig, {'display': 'none'}
        ...
        @app.callback(
            [Output('latency-process-button', 'disabled'),
             ...
             Output('latency-upload-status', 'children')],
            Input('latency-upload-data', 'contents'),
            State('latency-upload-data', 'filename'),
            prevent_initial_call=True
        )
        ...
        def process_file(n_clicks, uploaded_data, user_id):
            ...
            print(f"in process_file with trigger: {ctx.triggered_id}")
            print(f"n_clicks: {n_clicks}, uploaded_data: {uploaded_data}")

            if not n_clicks or not uploaded_data:
                return (
                    {'display': 'block'}, "", False, False, 0,
                    True, True,
                    #empty_fig, empty_fig, empty_fig, empty_fig,
                    None, None, None, None,
                    "", "", ""
        ...
        def process_file(n_clicks, filepath, user_id):
            empty_fig = go.Figure()
            ...
            print(f"in process_file with trigger: {ctx.triggered_id}")
            print(f"n_clicks: {n_clicks}, filepath: {filepath}")
            print(f"Checking: {filepath}")
            print("Absolute path:", os.path.abspath(filepath))
            print("Directory contents:", os.listdir(os.path.dirname(filepath)))
            print(os.path.exists(filepath))

            if not n_clicks or not filepath or not os.path.exists(filepath):
                return (
                    {'display': 'none'},
                    dbc.Alert("No file to process or file not found", color="danger", dismissable=True),
                    False, False, 0,
                    True, True,
                    empty_fig, empty_fig, empty_fig, empty_fig,
                    "", "", ""
            ...
            try:
                ...
                with open(filepath, 'r') as file:
                    csv_path, df = parse_log_file(file)
                total_cmd_count = len(df)
                ...
                self.user_data[user_id] = {
                    ...
                    'viz_builder': None,
                    'current_range': [0, min(self.default_range_size, total_cmd_count)],
                    'filepath': filepath  # Store filepath for cleanup later
                }

                return (
                    {'display': 'block'},
                    html.Div([
                        html.H5(f'Successfully parsed: {os.path.basename(filepath)}', style={'color': 'green'}),
                        html.P(f"Total commands: {total_cmd_count:,}"),
                        html.Hr()
                    ]),
                    True, True, total_cmd_count,
        ...
        # Load selected range when button is clicked
        @app.callback(
        ...
        def load_selected_range(load_clicks, next_clicks, initialized,
                                ...
                                current_range, total_cmd_count, user_id):
            ...
            # Initial load - use default range
            ...
            # Next range button
                ...
                if start >= total_cmd_count:
                    return no_update, dbc.Alert(
                        "Already at the end of the command log.",
                        color="warning",
                        dismissable=True
                    ), no_update

            # Load range button - check dropdown first, then custom inputs
            else:
                if dropdown_value:
                    start, end = map(int, dropdown_value.split(','))
                elif custom_start is not None and custom_end is not None:
                    start = custom_start
                    end = custom_end

                    # Validate custom range size
                    if end - start > self.default_range_size:
                        return no_update, dbc.Alert(
                            f"Custom range too large. Maximum size is {self.default_range_size:,} commands.",
                            color="danger",
                            dismissable=True
                        ), no_update
                else:
                    return no_update, dbc.Alert(
                        "Please select a range from the dropdown or enter a custom range.",
                        color="warning",
                        dismissable=True
                    ), no_update
            ...
            try:
                # Get the full dataframe and filter by command index range
                ...
                # Initialize data processor with the range data
                ...
                return [start, end], dbc.Alert(
                    f"Successfully loaded commands {start:,} to {end:,}",
                    color="success",
                    dismissable=True
                ), None
                ...
                return no_update, dbc.Alert(
                    f"Error processing range: {str(e)}",
                    color="danger",
                    dismissable=True
                ), no_update
